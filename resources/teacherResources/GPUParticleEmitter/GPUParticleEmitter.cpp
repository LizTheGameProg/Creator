...
void GPUParticleEmitter::createDrawShader()
{
	...
	FILE* file = fopen("./data/shaders/gpuParticle.vert", "rb");
	if (file == nullptr)
		return;
	...
	file = fopen("./data/shaders/gpuParticle.frag", "rb");
	fseek(file, 0, SEEK_END);
	length = ftell(file);
	fseek(file, 0, SEEK_SET);
	...
	// read the fragment shader
	file = fopen("./data/shaders/gpuParticle.geom", "rb");
	fseek(file, 0, SEEK_END);
	length = ftell(file);
	fseek(file, 0, SEEK_SET);
	source = new char[length + 1];
	...
	glShaderSource(gs, 1, (const char**)&source, 0);
	glCompileShader(gs);
	delete[] source;
...
{
	// read the shader
	FILE* file = fopen("./data/shaders/gpuParticleUpdate.vert", "rb");
	fseek(file, 0, SEEK_END);
	unsigned int length = ftell(file);
	fseek(file, 0, SEEK_SET);
	char* source = new char[length + 1];
