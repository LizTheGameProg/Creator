...
public:
	...
	SpriteBatch_GL3(AIE_GL::Application *pApp);
...
private:
	...
	void GetOrtho(float *mat4x4, float left, float right, float bottom, float top, float a_fNear, float a_fFar)
	{
		...
		mat4x4[12] = -1.f*((right + left)/(right - left));
		mat4x4[13] = -1.f*((top + bottom)/(top - bottom));
		mat4x4[14] = -1.f*((a_fFar + a_fNear)/(a_fFar - a_fNear));
		mat4x4[15] = 1.f;
	}
