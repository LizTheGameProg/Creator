...
void ParticleEmitter::initalise(unsigned int a_maxParticles, 
				   ...
				   const glm::vec4& a_startColour, const glm::vec4& a_endColour, float a_one)
{
	m_texture = SOIL_load_OGL_texture("./Build/textures/Star.tga", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
...
{
	unsigned int vs = Utility::loadShader("./Build/shaders/particle.vert", GL_VERTEX_SHADER);
	unsigned int fs = Utility::loadShader("./Build/shaders/particle.frag", GL_FRAGMENT_SHADER);
	unsigned int gs = Utility::loadShader("./Build/shaders/particle.geom", GL_GEOMETRY_SHADER);
...
void ParticleEmitter::createUpdateShader()
{
	...
	unsigned char* source = Utility::fileToBuffer("./Build/shaders/particleUpdate.vert");
	...
	location = glGetUniformLocation(m_updateShader,"one");
	glUniform1f(location, m_one);
}

void ParticleEmitter::draw(const glm::mat4& a_cameraTransform, const glm::mat4& a_projection)
{
	glEnable(GL_BLEND);
